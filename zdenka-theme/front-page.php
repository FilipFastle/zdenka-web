...
<div class="zc-home-hero" id="zcHomeHero">
    ...
    ?>
    <?php if($zc_hero_img): ?>
    <div class="zc-hero-bg" style="background-image:url('<?php echo esc_url($zc_hero_img); ?>')"></div>
    <?php else: ?><div class="zc-hero-bg" style="background:#1C1A18"></div><?php endif; ?>
...
<div class="zc-container" style="display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:center">
    <div>
        <div class="zc-eyebrow" style="color:rgba(184,164,122,.8)">Nezáväzná konzultácia</div>
        <h2 style="font-family:var(--serif);color:#fff;margin-bottom:16px">Predávate alebo hľadáte <em style="color:var(--accent)">nový domov?</em></h2>
        <p style="color:rgba(255,255,255,.65);font-size:16px;line-height:1.75;margin-bottom:32px">Prvá konzultácia je bezplatná a nezáväzná.</p>
        <?php foreach([
        ...
        <a href="<?php echo $href; ?>" style="display:flex;align-items:center;gap:12px;color:rgba(255,255,255,.75);font-size:15px;text-decoration:none;margin-bottom:14px">
            <span style="width:44px;height:44px;background:rgba(255,255,255,.1);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0"><?php echo $ic; ?></span>
            <?php echo esc_html($lbl); ?>
        </a>
        <?php endforeach; ?>
    </div>
    <div style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:var(--r-lg);padding:40px">
        <h3 style="font-family:var(--serif);color:#fff;font-size:22px;margin-bottom:24px">Napíšte mi správu</h3>
        <form id="zcContactForm">
            <?php foreach([['name','text','Vaše meno *'],['email','email','E-mail *'],['phone','tel','Telefón']] as [$n,$t,$p]): ?>
            <div style="margin-bottom:12px"><input type="<?php echo $t; ?>" name="<?php echo $n; ?>" placeholder="<?php echo $p; ?>" style="background:rgba(255,255,255,.08);border:1.5px solid rgba(255,255,255,.15);color:#fff;padding:13px 16px;border-radius:var(--r-sm);width:100%;font-family:var(--sans);font-size:16px" <?php if(substr($p, -strlen('*')) === '*') echo 'required'; ?>></div>
            <?php endforeach; ?>
            <div style="margin-bottom:16px"><textarea name="message" placeholder="Správa" rows="4" style="background:rgba(255,255,255,.08);border:1.5px solid rgba(255,255,255,.15);color:#fff;padding:13px 16px;border-radius:var(--r-sm);width:100%;font-family:var(--sans);font-size:16px;resize:vertical"></textarea></div>
